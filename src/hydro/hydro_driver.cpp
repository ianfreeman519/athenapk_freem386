...
void AddSTSTasks(TaskCollection *ptask_coll, Mesh *pmesh, BlockList_t &blocks,
  ...
  if (parthenon::Globals::my_rank == 0) {
    ...
    if (ratio > 800.1) {
      std::cout << "WARNING: ratio is > 800. Proceed at own risk." << std::endl;
    }
  }
...
TaskCollection HydroDriver::MakeTaskCollection(BlockList_t &blocks, int stage) {
  ...
  for (int i = 0; i < num_partitions; i++) {
    ...
    // Add non-operator split source terms, including the optional coupled
    // ohmic-heating/tabular-cooling thermal source handled inside AddUnsplitSources.
