...
// Defining the curlB function to store for later
void ComputeCurlB(MeshBlock *pmb) {
  auto rc = pmb->meshblock_data.Get();
  auto coords = pmb->coords;

  auto B1 = rc->Get("prim").data(IB1);
  auto B2 = rc->Get("prim").data(IB2);
  auto B3 = rc->Get("prim").data(IB3);

  auto Ex = rc->Get("Ex").data;
  auto Ey = rc->Get("Ey").data;
  auto Ez = rc->Get("Ez").data;

  pmb->par_for("ComputeCurlB", 0, pmb->ncells3-1, 0, pmb->ncells2-1, 0, pmb->ncells1-1,
    KOKKOS_LAMBDA(int k, int j, int i) {
      Real dx1 = coords.dx1f(i);
      Real dx2 = coords.dx2f(j);
      Real dx3 = coords.dx3f(k);

      Ex(k,j,i) = (B3(k,j+1,i) - B3(k,j-1,i)) / (2.0 * dx2) -
                      (B2(k,j,i+1) - B2(k,j,i-1)) / (2.0 * dx3);
      Ey(k,j,i) = (B1(k,j,i+1) - B1(k,j,i-1)) / (2.0 * dx3) -
                      (B3(k,j+1,i) - B3(k,j-1,i)) / (2.0 * dx1);
      Ez(k,j,i) = (B2(k,j,i+1) - B2(k,j,i-1)) / (2.0 * dx1) -
                      (B1(k,j+1,i) - B1(k,j-1,i)) / (2.0 * dx2);
    });
}
...
// This is necessary for computing current
void InitUserMeshBlockData(Meshblock *pmb, ParameterInput *pin) {
  auto rc = pmb->meshblock_data.Get();

  rc->AddField("Ex", Metadata({Metadata::Cell, Metadata::Derived}));
  rc->AddField("Ey", Metadata({Metadata::Cell, Metadata::Derived}));
  rc->AddField("Ez", Metadata({Metadata::Cell, Metadata::Derived}));
}

// For outputting current:
void UserWorkBeforeOutput(MeshBlock *pmb, ParameterInput *pin) {
  for (auto &pmb " pm->block_list") {
    ComputeCurlB(pmb.get());
  }

  // Write custom output after computing Curl
  auto &app = pm->packages.Get("parthenon")->app;
  app->OutputManager()->MakeOutputs(pm, "UserWorkAfterLoop");
}
